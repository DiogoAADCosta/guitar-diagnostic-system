...
#
# Goal:
# Improve the quality and fluency of the generated diagnostic text,
# making it more natural, varied, and closer to human feedback.
#
# In this version:
# - Introduced dynamic sentence connectors for natural phrasing
# - Reduced repetition in diagnostic output
# - Improved cause-and-effect sentence structure
# - Added connector variation for both strengths and limitations
# - Introduced minimum frequency threshold (>=2) for dominant signals
# - Expanded profile system with new pattern variations
# - Split additional observations into structured sections
#
# Focus:
# Transition from structured output → natural language generation
...
# This flag is activated when the user does not know what a barre chord is
# (Answer A in question 3 or question 5)
...
for questao in lista_perguntas:
    ...
    # questions 5 and 7 are skipped and a 'base' limitation is assumed
    #
    # questao.get() safely checks if 'skip_if' exists before comparing
    ...
    # Input validation is not handled here because in the final version
    # the user will select options via UI (frontend)
    resposta_usuario = input('\nSua Resposta: ').strip().upper()


    # If the user does not know what a barre chord (questions 3 or 5),
    # activate the skip logic for dependent questions
    if (questao['id'] == 3 and resposta_usuario == 'A') or (questao['id'] == 5 and resposta_usuario == 'A'):
        nao_sabe_pestana = True

    for alternativa in questao['respostas']:
        # Question 9 supports multiple answers, so multiple tags can be collected
        if questao['id'] == 9:
...
lista_nivel = {
    'iniciante': [ # Multiple phrasing options for randomized output (random.choice)
...
# Sentence connectors for dynamic natural language generation
conectores_cap = [
    'Você já ',
    'Você também ',
...
# Primary tags drive the main diagnosis
# Secondary tags are handled later as additional observations
...
# Select dominant tags (frequency >= 2) to reduce noise
limitadores_mais_frequentes = []
for item, quantidade in contador_limitadores.items():
    if quantidade == maior_frequencia_limitadores and quantidade >= 2 and item in limitadores_principais:  # Filtering only primary limitation tags (secondary ones are handled later)
        limitadores_mais_frequentes.append(item)
capacidades_mais_frequentes = []
for item, quantidade in contador_capacidades.items():
    if quantidade == maior_frequencia_capacidades and quantidade >= 2 and item in capacidades_principais: # Filtering only primary capacity tags (secondary ones are handled later)
...
# Determine user level based on balance between limitations and capabilities
nivel = ''
...
# Track connector usage to avoid repetitive sentence structure
conectores_cap_contador = 0
conectores_lim_contador = 0


# Attempt profile-based diagnosis using dominant tag patterns
perfil = ''
...
if perfil == '':
    ...
    if 'montagem_de_acorde' in limitadores_mais_frequentes and 'troca_de_acorde' in limitadores_mais_frequentes:
        ...
        elif (conectores_cap_contador != 0 and conectores_lim_contador == 0) or (conectores_cap_contador == 0 and conectores_lim_contador == 1):
        ...
        for limitador in limitadores_mais_frequentes:
            if conectores_cap_contador == 0 and conectores_lim_contador == 0:
                print(conectores_lim[min(conectores_lim_contador, len(conectores_lim) - 1)], end='')
            elif (conectores_cap_contador != 0 and conectores_lim_contador == 0) or (
                    conectores_cap_contador == 0 and conectores_lim_contador == 1):
                conectores_lim_contador += 1
                print(conectores_lim[min(conectores_lim_contador, len(conectores_lim) - 1)], end='')
            else:
                print(conectores_lim[min(conectores_lim_contador, len(conectores_lim) - 1)], end='')
...
# Experimental function (not used in current version)
