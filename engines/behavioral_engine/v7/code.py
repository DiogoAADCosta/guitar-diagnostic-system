...
lista_perguntas = [
     ...
     'respostas':  [{'resposta': 'A', 'tag': [['limitador', 'base']], 'texto': 'Não consigo montar nenhum acorde'},                        #Aqui coloquei lista dupla dentro de 'tag', pois podem existir mais de uma tag
...
# Funções para definir os conectores dos textos
def conector_cap(conectores_cap_contador):
    conector = conectores_cap[min(conectores_cap_contador, len(conectores_cap) - 1)]      # Para pegar o último item de conectores_cap caso o contador ultrapasse o tamanho da lista
    return conector
...
def conector_lim(conectores_lim_contador):
    if conectores_cap_contador == 0 and conectores_lim_contador == 0:
        ...
        print(f'Conectores lim dentro da função antes: {conectores_lim_contador}')
        conectores_lim_contador += 1
        print(f'Conectores lim dentro da função depois: {conectores_lim_contador}')
        conector = conectores_lim[min(conectores_lim_contador, len(conectores_lim) - 1)]
        return conector
    else:
        conector = conectores_lim[min(conectores_lim_contador, len(conectores_lim) - 1)]
...
if 'resistencia' in lista_limitadores_respostas_usuario:
    print(f'Conectores lim antes da função: {conectores_lim_contador}')
    print(conector_lim(conectores_lim_contador), end='')
    ...
    print(f'Conectores lim depois da função antes contador: {conectores_lim_contador}')
    conectores_lim_contador += 1
    print(f'Conectores lim depois da função depois contador: {conectores_lim_contador}')
